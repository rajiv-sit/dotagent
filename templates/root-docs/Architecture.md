...
This fits the current repo because the runtime already stores local job state and shells out through a local assistant CLI shim such as `agent.cmd`. A local DAG is enough for milestone-oriented workflows and keeps operational complexity low.
