...
class Orchestrator:
    ...
    def prepare_task(
        ...
        job.metadata["plan_id"] = plan.id
        plan.metadata["job_id"] = job.id
            ...
            summaries.append(
                {
                    ...
                    "agent_roles": output.get("agent_roles"),
                }
            )
    ...
    def result(self, job_id: str) -> Dict[str, Any]:
        ...
        output = job.get("output") or {}
        return {"job": job, "evidence": evidence, "telemetry": telemetry, "agent_roles": output.get("agent_roles")}
    ...
    def cancel(self, job_id: str) -> Dict[str, Any]:
        ...
        plan = None
        plan_id = job.get("metadata", {}).get("plan_id")
        if plan_id:
            plan = self.store.load_plan(plan_id)
        ...
        updated_job = self.store.update_job(job_id, status="CANCELLED")
        if plan_id and plan:
            for step in plan.get("steps", []):
                if step.get("status") in {"PENDING", "RUNNING"}:
                    step["status"] = "CANCELLED"
                    step["last_error"] = "Cancelled by user"
            metadata = dict(plan.get("metadata", {}))
            metadata["cancelled"] = True
            metadata["cancelled_at"] = utc_now()
            plan["metadata"] = metadata
            plan = self.store.update_plan(plan_id, **plan)

        self.store.emit_event("job_cancelled", {"job_id": job_id, "plan_id": plan_id})
        return {"job": updated_job, "plan": plan}
