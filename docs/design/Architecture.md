...
- PowerShell wrappers:
  ...
  - forward supported runtime options directly so wrapper behavior stays aligned with the Python CLI
- JSON persistence:
...
- installer smoke checks confirm the installed `.agent/runtime/` tree is runnable and excludes generated Python cache artifacts.
