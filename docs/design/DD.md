...
- `StepExecutor`
  - responsibility: tool dispatch, per-step attempt execution, bounded parallel execution, and structured failure conversion for tool exceptions
- `Planner`
...
- PowerShell wrapper script
  - responsibility: resolve the installed runtime package location, preserve legacy flags, forward supported runtime options, suppress Python bytecode cache writes, and invoke `python -m dotagent_runtime.cli`
...
### Tool failure conversion

1. Resolve the requested tool from the registry.
2. If lookup or execution raises an exception, create an `ExecutionResult` with `ok = false`.
3. Include the exception type and message in `stderr` and metadata.
4. Return the structured result to the normal validator path so plan state, evidence, telemetry, and events remain available.
...
- installer exclusion set for generated Python cache files and transient directories
...
- Treat tool registry lookup and execution exceptions as failed execution results so finalization still writes evidence
- Reject unsupported execution-target values at the wrapper boundary before invoking Python
- Restore `PYTHONDONTWRITEBYTECODE` after wrapper execution so caller shell state is preserved
