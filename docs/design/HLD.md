...
- PowerShell compatibility wrapper
  ...
  - passes supported runtime options such as command payloads, execution targets, and serial execution through to the Python CLI
- State store
...
- Executor
  ...
  - captures tool dispatch failures as structured execution failures instead of letting one exception abort finalization
- Validator
...
- installer verification can inspect `.agent/runtime/dotagent_runtime` without seeing generated cache artifacts
