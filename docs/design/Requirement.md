...
- FR-21: PowerShell wrappers must expose supported Python runtime options without silently dropping execution commands, target selection, or serial execution controls.
- FR-22: Installer copy operations must exclude generated cache artifacts such as `__pycache__` directories and `.pyc` files from consumer project installs.
- FR-23: Runtime tool dispatch must convert tool-level exceptions into structured execution results so orchestration can persist validation, telemetry, evidence, and failure context.
- FR-24: PowerShell wrappers must prevent Python bytecode cache generation during normal runtime invocation so installed consumer runtime trees remain clean after execution.
...
- Install cleanliness: generated runtime caches must not be copied into consumer repos.
- Compatibility: PowerShell and Python command surfaces must remain aligned for supported task and run options.
...
- Missing or broken tools must fail as structured step failures rather than crashing before evidence is written
- Installed runtime trees must not include interpreter-generated cache files
- Wrapper-supported arguments must reach the Python CLI consistently
