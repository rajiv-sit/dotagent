...
Harden the runtime and installer path for wrapper parity, clean installs, and structured failure persistence.
...
- `runtime/dotagent_runtime/executor.py`
- `runtime/tests/test_orchestrator.py`
- `runtime/tests/test_executor.py`

### Verification Steps

- Run Python runtime unit tests
- Run PowerShell wrapper smoke checks for task command forwarding
- Run installer smoke check and confirm copied runtime excludes `__pycache__` and `.pyc`
- Run installed consumer smoke check and confirm runtime commands do not create `__pycache__` or `.pyc`
- Run markdown link validation and health check

### Exit Criteria

- PowerShell task/run options are forwarded to the Python runtime
- consumer installs do not copy generated Python cache artifacts
- normal wrapper invocation does not generate Python cache artifacts in installed runtime trees
- tool dispatch exceptions become persisted failed steps with evidence and telemetry
